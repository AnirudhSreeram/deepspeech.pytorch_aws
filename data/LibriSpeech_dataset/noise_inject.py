...
data = load_audio(os.path.normpath(args.input_path))
mixed_data = noise_injector.inject_noise_sample(data, os.path.normpath(args.noise_path), args.noise_level)
mixed_data = torch.tensor(mixed_data, dtype=torch.float).unsqueeze(1)  # Add channels dim
write(filename=os.path.normpath(args.output_path),
